      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {customers.map((c) => {
            ...
            return (
              <tr key={c.id} className="border-b border-[#F1EFE8] last:border-0">
                <td className="py-3 pr-4 text-[#2C2C2A] font-medium">{c.customerCode}</td>
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[#2C2C2A]">{c.name}</span>
                  </div>
                </td>
                <td className="py-3 pr-4 text-[#5F5E5A]">{c.phone}</td>
                <td className="py-3 pr-4 text-[#5F5E5A]">{maskAadhaar(c.aadhaarNumber)}</td>
                <td className="py-3 pr-4 text-[#5F5E5A]">{c.city ?? "—"}</td>
                <td className="py-3 pr-4"><StatusBadge status={c.status} /></td>
                <td className="py-3 pr-4 text-[#5F5E5A]">{formatDate(c.createdAt)}</td>
